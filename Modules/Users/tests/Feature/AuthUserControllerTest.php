...
test('only_guest_can_reset_password', function () {
    ...
    postJson(route('api.auth.forget-password'))->assertJsonValidationErrorFor('email');
    postJson(route('api.auth.reset-password'))->assertJsonValidationErrorFor('email')->assertJsonValidationErrorFor('token');
});

test('user_cannot_reset_password_with_invalid_data', function () {
    asGuest();

    postJson(route('api.auth.forget-password'), ['email' => fake()->email])->assertStatus(400)->assertSee(__('users::user.email_not_found'));
    postJson(route('api.auth.reset-password', [
        'email' => fake()->email,
        'password' => $password = str()->random(8),
        'password_confirmation' => $password,
        'token' => str()->random(6),
    ]))->assertStatus(400)->assertSee(__('users::user.invalid_credentials'));

    $user = User::factory()->customer()->create();
    postJson(route('api.auth.forget-password'), ['email' => $user->email])->assertNoContent();
    ...
    ]))->assertStatus(400)->assertSee(__('users::user.token_expired'));
});
...
test('user_can_reset_password', function () {
    ...
    expect(Customer::attempt(['email' => $user->email, 'password' => $password]))->toBeTrue();
